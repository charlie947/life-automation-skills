...
- Any request to find, review, or resolve duplicate or half-filled records in Airtable or Notion.
...
- **Neither connector can merge.** There is no "merge two records" call. A merge is: read both, compose the merged record, **write the survivor first, then retire the loser second.** The skill always does it in that order so nothing is lost if it stops midway.
- **Deletes barely undo.** Airtable can revert only its most recent action. Notion cannot delete at all through the connector. So the skill proposes, confirms, then writes.
...
1. **Confirm the target.** Which base and table (Airtable) or which database (Notion). If unclear, list the candidates and ask. Never guess the table.
...
9. **On the user's yes, apply the confirmed changes only.** Write survivors first, then retire losers (Airtable delete, or Notion flag-and-archive-in-UI). Report exactly what was written and what is still awaiting the user.
...
## Keywords
crm, crm hygiene, clean my crm, tidy my crm, dedupe, deduplicate, duplicate contacts, merge duplicates, merge contacts, fix my contact list, contact cleanup, enrich contacts, tidy contacts, normalise contacts, Airtable, Notion, contacts, records, data cleanup
