...
Do **not** use it to send email (Claude's Gmail connector cannot send, see below), to chase invoices or overdue payments (that's `invoice-chaser`), or to triage the whole inbox (that's `inbox-executive-assistant`). This skill only looks outward at what *others* owe the user.

## What it needs (setup)

Two native Claude connectors, both login-only OAuth, no build:

- **Gmail connector**: search (including `in:sent`), read threads, and create drafts. It **cannot send**. Every nudge is left in Drafts for the user to review and send.
...
1. **Never send. Draft only.** Every nudge lands in Drafts for the user to read and send. The connector can't send anyway, never route around that with another tool unless the user has explicitly wired and approved a send path.
...
7. **Least privilege.** Use only the Gmail and Calendar connectors. Do not reach for other tools, accounts, or contact lists.
...
5. **Match the user's voice.** Read the user's own side of the thread and their other `in:sent` mail to the same person for tone and sign-off. Never invent a house style. Keep any genuine unknown as a bracketed `[NEEDS: …]` for the user to fill. Never guess a fact, figure, or date.
...
## Keywords
follow-up, follow up, chase, chaser, nudge, who owes me a reply, what am I waiting on, waiting on others, outstanding, no reply, unanswered, went quiet, stalled thread, ping them, circle back, chase that up, in:sent, Gmail, calendar, before the meeting
